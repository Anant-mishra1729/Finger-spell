...
class HandTracker:
    ...
    def drawHands(self, image, landmarks):
        self.draw_module.draw_landmarks(
            image, landmarks, self.hands_module.HAND_CONNECTIONS
        )
    ...
    def getPos(self, landmark, shape, points=[]):
        ...
        if not len(points):
            return [
                (int(lmList[i].x * width), int(lmList[i].y * height))
                for i in range(len(lmList))
            ]
        else:
            return [
                (int(lmList[i].x * width), int(lmList[i].y * height)) for i in points
            ]
